...
mdf <- 
  ...
  ungroup %>% 
  mutate(x=rep(0,8),
         y=c(300,.3,300,.3,300,.3,300,.3),
         label=paste('slope =',format(m,digits=2))
  )
...
plot_cbd=function(dF,errorvar){
  ...
  ggplot()+
    geom_ribbon(aes(x=fscamae,ymin=l,ymax=u), fill='grey90')+
    geom_path(aes(x=fscamae,y=avg))+
    geom_text(data=mdf1,aes(x,y,label=label),hjust=0)+
...
pg
save_plot(plot=pg,filename = 'figs/xdate_meanfit.eps',nrow=2, base_width=12)
...
makeplot_fun=function(mdlpreds,errorvar,valtype,data){
  ...
  if(valtype=='diff'){
    if(errorvar=='r2')  {
      ...
      ax=theme(axis.line.x=element_blank())
    }
    if(errorvar=='pctmae')  {
  ...
  data %>% 
    ggplot()+
    geom_bar(aes(x=simdte,y=val,fill=var),stat='identity',position='dodge',show_guide=FALSE)+
    facet_grid(~simyr,scales='free_x')+
...
phv_pctmae <- plot_grid(gglist[[2]],
                        gglist[[1]],
                        nrow=2,
                        rel_heights = c(.5,1),
                        align='v')
phvaso_pctmae <- plot_grid(gglist[[4]],
                        gglist[[3]],
                        nrow=2,
                        rel_heights = c(.5,1),
                        align='v')
phvasofsca_pctmae <- plot_grid(gglist[[6]],
                        gglist[[5]],
                        nrow=2,
                        rel_heights = c(.5,1),
                        align='v')
phvfsca_pctmae <- plot_grid(gglist[[8]],
                        gglist[[7]],
                        nrow=2,
                        rel_heights = c(.5,1),
                        align='v')
phv_r2 <- plot_grid(gglist[[10]],
                        gglist[[9]],
                        nrow=2,
                        rel_heights = c(.5,1),
                        align='v')
phvaso_r2 <- plot_grid(gglist[[12]],
                        gglist[[11]],
                        nrow=2,
                        rel_heights = c(.5,1),
                        align='v')
phvasofsca_r2 <- plot_grid(gglist[[14]],
                        gglist[[13]],
                        nrow=2,
                        rel_heights = c(.5,1),
                        align='v')
phvfsca_r2 <- plot_grid(gglist[[16]],
                        gglist[[15]],
                        nrow=2,
                        rel_heights = c(.5,1),
                        align='v')
